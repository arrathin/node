...
ChildProcess.prototype.spawn = function(options) {
  ...
  if (process.platform === 'os390') {
    ...
    if (process.env._BPXK_AUTOCVT)
      zOS_options.push('_BPXK_AUTOCVT=' + process.env._BPXK_AUTOCVT);
    if (process.env._CEE_RUNOPTS)
      zOS_options.push('_CEE_RUNOPTS=' + process.env._CEE_RUNOPTS);
    if(process.env._TAG_REDIR_ERR)
      zOS_options.push('_TAG_REDIR_ERR=' + process.env._TAG_REDIR_ERR);
    if (process.env._TAG_REDIR_IN)
     zOS_options.push('_TAG_REDIR_IN=' + process.env._TAG_REDIR_IN);
    if (process.env._TAG_REDIR_OUT)
      zOS_options.push('_TAG_REDIR_OUT=' + process.env._TAG_REDIR_OUT);
    if (process.env.STEPLIB)
      zOS_options.push('STEPLIB=' + process.env.STEPLIB);
    if (process.env.LIBPATH)
      zOS_options.push('LIBPATH=' + process.env.LIBPATH);

    options.envPairs = zOS_options.concat(options.envPairs || []);
  }
